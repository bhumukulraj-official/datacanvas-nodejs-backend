...
| API Endpoint | HTTP Method | Database Table | Primary Operation |
|--------------|-------------|----------------|-------------------|
| `/api/v1/auth/register` | POST | `users` | INSERT |
| `/api/v1/auth/login` | POST | `users`, `refresh_tokens` | SELECT, INSERT |
| `/api/v1/auth/refresh` | POST | `refresh_tokens` | SELECT, UPDATE |
| `/api/v1/profile` | GET | `profiles` | SELECT |
| `/api/v1/profile` | PUT | `profiles` | UPDATE |
| `/api/v1/profile/avatar` | POST | `profiles`, `media` | UPDATE, INSERT |
| `/api/v1/profile/resume` | POST | `profiles`, `media` | UPDATE, INSERT |
| `/api/v1/projects` | GET | `projects` | SELECT |
| `/api/v1/admin/projects` | POST | `projects` | INSERT |
| `/api/v1/admin/projects/:id` | PUT | `projects` | UPDATE |
| `/api/v1/admin/projects/:id` | DELETE | `projects` | DELETE |
| `/api/v1/blog/posts` | GET | `blog_posts` | SELECT |
| `/api/v1/blog/posts/:slug` | GET | `blog_posts` | SELECT |
| `/api/v1/admin/blog/posts` | POST | `blog_posts` | INSERT |
| `/api/v1/admin/blog/posts/:id` | PUT | `blog_posts` | UPDATE |
| `/api/v1/admin/blog/posts/:id` | DELETE | `blog_posts` | DELETE |
| `/api/v1/contact` | POST | `contact_submissions` | INSERT |
| `/api/v1/notifications` | GET | `notifications` | SELECT |
| `/api/v1/notifications/:id/read` | PUT | `notifications` | UPDATE |
| `/api/v1/media/upload` | POST | `media` | INSERT |
| `/api/v1/media` | GET | `media` | SELECT |
| `/api/v1/media/:id` | DELETE | `media` | DELETE |

### Field Constraints

| Table | Field | API Constraint | Database Constraint |
|-------|-------|----------------|---------------------|
| `users` | `email` | Required, email format | VARCHAR(255), UNIQUE, NOT NULL |
| `users` | `password` | Min: 8 chars, uppercase, lowercase, number, special char | VARCHAR(255), NOT NULL |
| `users` | `name` | Max: 100 chars | VARCHAR(100), NOT NULL |
| `profiles` | `title` | Max: 200 chars | VARCHAR(200) |
| `profiles` | `bio` | Max: 1000 chars | TEXT |
| `profiles` | `phone` | Max: 20 chars | VARCHAR(20) |
| `profiles` | `location` | Max: 100 chars | VARCHAR(100) |
| `projects` | `title` | Max: 200 chars | VARCHAR(200), NOT NULL |
| `projects` | `description` | Required | TEXT, NOT NULL |
| `blog_posts` | `title` | Max: 200 chars | VARCHAR(200), NOT NULL |
| `blog_posts` | `slug` | Required, unique | VARCHAR(200), UNIQUE, NOT NULL |
| `blog_posts` | `content` | Required | TEXT, NOT NULL |
| `blog_posts` | `excerpt` | Max: 500 chars | VARCHAR(500) |
| `contact_submissions` | `name` | Max: 100 chars | VARCHAR(100), NOT NULL |
| `contact_submissions` | `email` | Required, email format | VARCHAR(255), NOT NULL |
| `contact_submissions` | `subject` | Max: 200 chars | VARCHAR(200), NOT NULL |
| `contact_submissions` | `message` | Max: 1000 chars | TEXT, NOT NULL |

### Caching Strategy

| Endpoint | Cache Key | TTL | Invalidation Trigger |
|----------|-----------|-----|----------------------|
| `/api/v1/profile` | `profiles:{user_id}` | 2 hours | Profile update |
| `/api/v1/projects` | `projects:list:{page}:{limit}:{sort}:{order}:{tags}` | 1 hour | Project create/update/delete |
| `/api/v1/projects/:id` | `projects:{id}` | 1 hour | Project update/delete |
| `/api/v1/blog/posts` | `blog_posts:list:{page}:{limit}:{category}:{tag}:{search}` | 30 minutes | Blog post create/update/delete |
| `/api/v1/blog/posts/:slug` | `blog_posts:slug:{slug}` | 30 minutes | Blog post update/delete |
| `/api/v1/settings` | `settings:public` | 24 hours | Settings update | 